...
def splitTransitStopsData(transitdata,transitDataLayer,countyDir):
    ...
    arcpy.SelectLayerByAttribute_management(transitDataLayer, "NEW_SELECTION", "\"access\" = 3")
...
def bufferTransitStops_QUARTER(transitstops, bufferedName, geoDataBaseName):
    ...
    countypath = dataPath +  countydir + "\\"
    arcpy.Buffer_analysis( countypath  + geoDataBaseName + "\\" + transitstops, countypath + "\\" + geoDataBaseName + "\\" + bufferedName, "1320 Feet")

def bufferTransitStops_HALF(transitstops, bufferedName, geoDataBaseName):
    print "bufferDestinations"
    countypath = dataPath +  countydir + "\\"
    arcpy.Buffer_analysis( countypath  + geoDataBaseName + "\\" + transitstops, countypath + "\\" + geoDataBaseName + "\\" + bufferedName, "2640 Feet")
...
# No Split required
def scoreparcels2(targetFeature,bufferedTransitStop,geoDataBaseName, halfMile, access, layerCount):
    layerCount = layerCount + 1
    parcel_layer = 'parcels_lyr' + str(layerCount)
