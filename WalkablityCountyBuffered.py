__author__ = 'dev'
# This version buffers the target county which is used to select all parcels in and near the county
# Then
import arcpy,os,sys
import time
...
def generateLayers(targetFeatureClass,targetFeature):
    ...
    arcpy.Split_analysis(homedir + "\\" + geoDataBaseName +  "\\targetFeatureScored", homedir + "\\" + geoDataBaseName +  "\\targetFeatureScored", "walkable", geoDataBaseOutputPath)
    logout.write('Split_analysis was called'+ '\n')
    createWalkabilityLayer(homedir + "\\" + geoDataBaseName + "\\targetFeatureScored","walk1")
    createWalkabilityLayer(homedir + "\\" + geoDataBaseName + "\\targetFeatureScored","walk2")
    createWalkabilityLayer(homedir + "\\" + geoDataBaseName + "\\targetFeatureScored","walk3")
    createWalkabilityLayer(homedir + "\\" + geoDataBaseName + "\\targetFeatureScored","walk4")
    createWalkabilityLayer(homedir + "\\" + geoDataBaseName + "\\targetFeatureScored","walk5")
'''


# NEW and needs integration
def loadAndMergeCountiesInMetro(metrocounties):  # These are from county shape files - not parcel shapefiles
    fieldMappings = None
    metro = metrocounties[0]
    for county in metrocounties:
        arcpy.Merge_management([metro, county], metro, fieldMappings)
    return metro

# NEW and needs integration
def findNeighboringCounties(fips,metro,output_neighbors_table):
    arcpy.PolygonNeighbors_analysis(metro, output_neighbors_table)
    # no do lookup in output_neighbors_table with fips

# NEW and needs integration
def getDestinationParcelsInAndNearCounty(county, geoDataBaseName,nameForBufferedCounty, nameForDestinations, destinationInMetroCounties):
    arcpy.Buffer_analysis(homedir + "\\" + geoDataBaseName + "\\" + county, homedir + "\\" + geoDataBaseName + "\\bufferedCounty" + nameForBufferedCounty,"2640 Feet")
    for destinations in destinationInMetroCounties:
        arcpy.MakeFeatureLayer_management(destinations, 'destinations_lyr')
        arcpy.SelectLayerByLocation_management('destinations_lyr', 'intersect', nameForBufferedCounty)
        arcpy.CopyFeatures_management(nameForBufferedCounty, 'targetDestinations')
        arcpy.Merge_management("targetDestinations",nameForDestinations)
    return nameForDestinations
...
if __name__ == '__main__':
    ...
    targetCounty_shp = homedir + "\\county.shp"
    ...
    if creategdb:
        if os.path.isdir(outdir + "\\walk.gdb"):
            ...
            gdbfiles = os.listdir(outdir + "\\walkability.gdb")
            for gf in gdbfiles:
    ...
    geoDataBaseOutputPath = homedir + "\\" + geoDataBaseOutput
    ...
    if multicounty2maps:
        ...
        #metrocounties = os.listdir(countiesfolder)
        metroList = [];
        for countyfolder in countyFolderList:
            metroList.append(countyfolder + "\\county.shp")
            nearParcelsList.append(countyfolder + "\\parcels.shp")
        if metroList > 1:
            metro = loadAndMergeCountiesInMetro(metroList)
        else:
            metro = metroList[0]
        # Get Neighboring counties using county shape files and touch
        findNeighboringCounties(targetCounty_shp,metro,"neighborsTable") # targetFeatures is fips
        metroDestinations = localParcels.name + "_metro_dest"  # name to use for parcels

        # Buffers county and selects all parcels inside the buffered county bourndary
        nearParcels = getDestinationParcelsInAndNearCounty(localParcels, geoDataBaseName, localParcels.name, localParcels.name + "_dest", nearParcelsList)

        selectDestinations(nearParcels,"distinationParcels") # may not be needed
        features2gdb(geoDataBaseName) # may not be needd
